(function () {
  function initApp() {
    ...
    if (stopBtn) stopBtn.addEventListener('click', function () { stopTone(false, configFadeOut); clearSleepTimer(); resetTimerUI(); });
    if (vol) {
    ...
    if (customSave) {
      customSave.addEventListener('click', function () {
        var obj = collectCustomFromModal(); if (!validateCustom(obj)) return; if (editingCustomId) { CUSTOMS = CUSTOMS.map(function (c) { if (c.id === editingCustomId) { return { id: c.id, name: obj.name, description: obj.description || '', freqs: obj.freqs, bg: obj.bg || '', freqVol: (typeof obj.freqVol === 'number' ? obj.freqVol : 100), timer: obj.timer || { value: '', hrs: 0, min: 0 } }; } return c; }); } else { var id = uuid(); CUSTOMS.push({ id: id, name: obj.name, description: obj.description || '', freqs: obj.freqs, bg: obj.bg || '', freqVol: (typeof obj.freqVol === 'number' ? obj.freqVol : 100), timer: obj.timer || { value: '', hrs: 0, min: 0 } }); }
